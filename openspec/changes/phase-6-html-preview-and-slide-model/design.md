...
### D3：主题 token 由 scene/styleProfile 确定性派生**并显式校验**
...
- `POST /projects/{id}/slides/materialize` 前置：`state==SLIDE_GENERATION` 且 **`spec is not None and spec.get("confirmedByUser")`**（None-safe，否则 `SLIDES_NOT_MATERIALIZABLE`/409——物化依赖 spec 的 scene/styleProfile 派生 theme 且 `validatePresentation` 要求 `scene===spec.scene`）且 `slidePlans` 非空 + `slidePlansConfirmed`（None-safe）。
