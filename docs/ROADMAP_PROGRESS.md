...
- 实现完成，待主控验收/归档：`Phase 2`（后端 API 与工作流状态机），change 位于 `openspec/changes/phase-2-api-skeleton-and-workflow-state`
- 当前后续：验收/归档 `Phase 2` 后启动 `Phase 3`（需求澄清与 Spec Builder）
...
| Phase 2 | 后端 API 与工作流状态机 | FastAPI、项目生命周期、状态流转、事件写入、错误约定 | 实现完成，待主控验收/归档 | `phase-2-api-skeleton-and-workflow-state` | 创建项目与状态推进可运行；非法状态不写持久状态 |
...
- 进行中的 change：`phase-2-api-skeleton-and-workflow-state`（实现完成，待主控验收/归档）
